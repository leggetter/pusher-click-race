 ...
 * and choosing how to export them on to a namespace.
 *
 * Example:
 *
 * com.pusher.namespace("com.pusher.util", function(export) {
 ...
 *                  functions and classes to be exported and added to the namespace.
 */
com.pusher.define = function(namespace, definition) {
  ...
  for(var thingToExport in export) {
    nsObject[thingToExport] = export[thingToExport];
  }
};
...
com.pusher.define("com.pusher", function(export) {
  ...
  Pusher.prototype = {
    ...
    subscribe: function(channel_name) {
      ...
      if (this.connection.state === 'connected') {
        this._authorize(channel, function(err, data) {
          if (err) {
    ...
    multiSubscribe: function(channels) {
      ...
      if(!this.options.multiAuth) {
        throw "the 'multiAuth' option must be set to tru for multiSubscribe to be allowed.";
      }
    ...
    /** @private */
    _authorize: function(channel, callback) {
      if(!this._requiresAuthorization(channel.name)) {
        callback(false, {});
      }
      else {
        var authData = this._createAuthRequestData(this.connection.socket_id, channel.name);
        var self = this;
        setTimeout(function() {
          // since a user might bind to the before_auth event we need to give the user the opportunity to do this
          // before making our auth request. Thus setTimeout.
          channel.dispatch("pusher:before_auth", authData);
        
          var sendData = "";
          for(var param in authData) {
            sendData += param + "=" + encodeURIComponent(authData[param]) + "&";
          }
          sendData = sendData.substring(0, sendData.length-1);
        
          self._makeRequest(Pusher.channel_auth_endpoint, sendData, callback, "Couldn't get auth info from your webapp");
        }, 0);
      }
    },
    
    /** @private */
    _createAuthRequestData: function(socketId, channelName) {
      var auth = {
        socket_id: socketId,
    ...
    /** @private */
    _requiresAuthorization: function(channelName) {
      return ( Pusher.Util.startsWith(channelName, 'private-') ||
    ...
    _multiAuth: function(channels, callback) {
      ...
      var self = this;
      setTimeout(function() {
        self.global_channel.dispatch("pusher:before_multi_auth", postData);
        self._makeRequest(Pusher.channel_multiauth_endpoint, postData, callback, "Couldn't get multiauth info from your webapp");
      }, 0);
    },
    
    /** @private */
    _makeRequest: function(url, sendData, callback, debugInfo) {
      
      var request = new com.pusher.RequestTypes[Pusher.channel_auth_transport];
      request.send(url, sendData, callback, this, debugInfo);
    },
    ...
    _filterAuthChannels: function(channels) {
      ...
      for(var i = 0, l = channels.length; i < l; ++i) {
        channelName = channels[i];
        if( this._requiresAuthorization(channelName) ) {
  ...
  Pusher.cdn_http = 'localhost:4500/dev/'
  Pusher.cdn_https = 'localhost:4500/dev/'
...
com.pusher.define("com.pusher", function(export) {
  ...
  AjaxRequest.prototype.send = function(url, sendData, callback, pusher, debugInfo) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState == 4) {
        if (xhr.status == 200) {
          ...
          Pusher.debug(debugInfo, status);
          callback(true, xhr.status);
        }
      }
    };
    var event = {xhr:xhr, data:sendData};
    pusher.global_channel.dispatch("pusher:before_xhr", event);
    xhr.send(event.data);
  };
  
  var JsonPRequest = function() {
    this._instanceId = JsonPRequest.instances.length;
    JsonPRequest.instances.push(this);
  };
  JsonPRequest.instances = [];
  
  JsonPRequest.prototype.send = function(url, sendData, callback, debugInfo) {
    
    this._callback = callback;
    this._debugInfo = debugInfo;
    
    var script = document.createElement("script");
    var callback_name = "com.pusher.JsonPRequest.instances[" + this._instanceId + "].callback";
    script.src = Pusher.channel_auth_endpoint+'?callback='+encodeURIComponent(callback_name)+'&'+sendData;
    var head = document.getElementsByTagName("head")[0] || document.documentElement;
    head.insertBefore( script, head.firstChild );
  };
  JsonPRequest.prototype.callback = function(data) {
    this._callback(data);
  };
  
  export.RequestTypes = {};
  export.RequestTypes.ajax = AjaxRequest;
  export.RequestTypes.jsonp = JsonPRequest;  
});
;(function() {
...
com.pusher.define("com.pusher", function(export) {
  ...
  Members.prototype.add = function(id, info) {
    this._members_map[id] = info;
    this.count++;
    return this.get(id);
  };

  Members.prototype.remove = function(user_id) {
  ...
  Members.prototype.get = function(user_id) {
...
com.pusher.define("com.pusher", function(export) {
  ...
  Pusher.Channel.PresenceChannel = {
    ...
    members: new com.pusher.Members()
  };
